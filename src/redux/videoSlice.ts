...
export interface video {
  ...
  lockRefresh: number | 0, // Lock refresh period
  lockState: boolean,         // Whether lock has been obtained
  lock: lockData,
  lockError: string | '',   // Lock error message
}
...
const videoSlice = createSlice({
  ...
  reducers: {
    ...
    setLockError: (state, action: PayloadAction<video["lockError"]>) => {
  ...
  extraReducers: builder => {
    ...
    builder.addCase(
      fetchVideoInformation.fulfilled, (state, action) => {
        ...
        state.lockError = action.payload.lock_error;
    })
    builder.addCase(
...
export const { setTrackEnabled, setIsPlaying, setIsPlayPreview, setCurrentlyAt, setCurrentlyAtInSeconds,
  ...
  setLock, setLockError, cut, markAsDeletedOrAlive, setSelectedWorkflowIndex, mergeLeft, mergeRight, setPreviewTriggered,
...
export const getLockError = (state: { videoState: { lockError: video["lockError"] }; }) => state.videoState.lockError
